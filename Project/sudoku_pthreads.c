...
int main(int argc, char *argv[]){
    ...
    // Allocate memory for board queue
    queue = (struct Pthread_data *)malloc(NUM_THREADS * 100 * sizeof(struct Pthread_data));
...
void *work(){
    ...
    while (!solution_found) {
        ...
        if (queue == NULL) {
            ...
            struct Pthread_data *job = queue;
            queue = NULL;
            // Unlock the mutex
            pthread_mutex_unlock(&mutex);
            // Do the work
            printf("Solving board from work function \n");
            printf("Input argumetns: %d, %d, %d %d \n", job -> board[0][0], n, N, job -> nr_remaining);


            if (solveBoard(job -> board, n, N, job -> nr_remaining, unassigned_indicies)){
                printf("Solution found apparently \n");
                pthread_mutex_lock(&mutex);
                solution_found = 1;
                pthread_mutex_unlock(&mutex);
            }
